...
module.exports = class Guild {
	constructor(guild, gocheClient = new GocheClient(), shard = 0) {
		...
		this.shardID = (this.id >> 22) % gocheClient.shard;
		this.roles = new ObjectManager();
		this.roleManager = new RoleManagerAction(gocheClient, this);
		this.emojis = new ObjectManager();
		this.members = new ObjectManager();
		this.channels = new ObjectManager();
		this.category = new ObjectManager();
		this.voiceChannels = new ObjectManager();
	...
	async fetchUser(id) {
		let user;
		await this.gocheClient.goche.requestManager.getRequest(
			`users/${id}`,
			(data) => {
				if (data.error === true) {
	...
	async fetchBans(id) {
		if (typeof id === 'string') {
			...
			throw Error(
				'Set the Argument to String (fetchUserBan[Guild])'
			);
		}
		return [];
	}
	...
	async fetchUserBan(id) {
		...
		return {
			...
			errorInfo: {
				message:
					"It seems that there was a leak of if and that was why there was this return. Don't worry, everything is fine. (fetchUserBan[Guild])",
			},
		};
	}
	...
	async removeBan(member) {
		...
		if (typeof member === 'string') {
			await this.fetchUserBan(member).then(
				async (e) => {
					if (e.error === true) {
						dataBan = e;
					} else {
			...
			throw Error(
				'Set the Argument to String (removeBan[Guild])'
			);
		}
		return {
			type: 'http/unknown',
		...
		if (typeof member === 'string') {
			...
			if (typeof memberGet === 'object') {
				this.gocheClient.goche.requestManager.otherRequest(
					'delete',
					`guilds/${this.id}/members/${memberGet.user.id}`,
					(data) => {
						if (
							data.error ===
							true
	...
	async selfSetNick(nickname, reason = '') {
		...
		await this.gocheClient.goche.requestManager.otherRequest(
			'patch',
			`guilds/${this.id}/members/@me/nick`,
			(data) => {
				if (data.error === true) {
	...
	modifyMember(id) {
		if (typeof id === 'string') {
			...
			throw Error(
				'Set the Argument to String (modifyMember[Guild])'
			);
		}
	}
};
