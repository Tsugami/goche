...
module.exports = class MemberGuildAction {
    constructor(gocheClient = new GocheClient(), guild = new Guild(), member = new Member()) {
    ...
    async done() {
        await this.gocheClient.goche.requestManager.otherRequest('patch', `guilds/${this.guild.id}/members/${this.member.user.id}`, (data) => {
            if (data.error === true) {
                return data
            }
