...
module.exports = class ConnectionManager {
	...
	ok(type, intents, id) {
		...
		this.ws.on('message', async (message) => {
			...
			switch (data.op) {
				case 11:
					this.websocketManager.gocheClient.ping =
						Date.now() -
						this.lantecy;
					break;
				case 10:
	...
	connect(type) {
		...
		this.ok(
			type,
			this.websocketManager.gocheClient.intentManager
				.intents === 0
				? false
				: true
		);
	...
	async identify(type, intents) {
		this.ready = true;
		this.websocketManager.gocheClient.goche.activities.setListening(
			`Sharding on ${this.shardID}`
		);
